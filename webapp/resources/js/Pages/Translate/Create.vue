...
<template>
    <Layout title="新規登録 | News" nav_active="news">
        <div class="font-sans">
            <p>
                <Link :href="route('news.index')" class="text-base md:text-sm text-green-500 no-underline hover:underline">News一覧</Link>
                <span class="ml-2 text-base md:text-sm text-gray-500">&gt; 新規登録</span>
            </p>
            <h1 class="font-bold font-sans break-normal text-gray-900 pt-6 pb-2 text-3xl md:text-2xl">新規登録</h1>
        </div>
        <form @submit.prevent="submit" class="mt-10">
            ...
            <button type="submit" :disabled="form.processing" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">登録する</button>
        </form>
    </Layout>
</template>
